...
4) Pay attention to control input saturation. How does it affect S(w)?
5) Masses and inertias are _inferred_ from the .xml geometry
6) Infer friction coefficient
7) Probably will have to establish all the parameters (mass, inertia, etc.) ourselves, then train a swingup policy, then do all this
8) Consider using cos(phi) and sin(phi) as states, instead of phi, to avoid the issue of changing from 360 deg to 0 deg

9) We NEED to offset the linearied model with the linearization point. How does S(w) change if our linear system starts from x(0) ?
"""
...
def test(cfg: dict) -> None:
	...
	for ii in range(dim_y):
		...
		rrtp_MO[ii].update_spectral_density(args_density,state_ind=ii)
