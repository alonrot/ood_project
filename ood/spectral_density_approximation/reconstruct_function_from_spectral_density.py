...
class ReconstructFunctionFromSpectralDensity(tf.keras.layers.Layer):
	...
	def train(self,Nepochs,learning_rate,stop_loss_val,lengthscale_loss,print_every=100):
		...
		# learning_rate_sche = tf.keras.optimizers.schedules.LearningRateSchedule(initial_learning_rate=learning_rate,
		# 																		decay_steps=10000,
		# 																		decay_rate=0.9)
		...
		while epoch < Nepochs and not done:
			...
			if (epoch+1) % print_every == 0:
				logger.info("    * Reconstruction loss (current): {0:.6f} [stopping loss: {1:.6f}]".format(tf.squeeze(loss_value),stop_loss_val))
				logger.info("    * Reconstruction loss (best):    {0:.6f} [stopping loss: {1:.6f}]".format(tf.squeeze(loss_value_best),stop_loss_val))
